...
**WienWasGeht** solves this by running an automated ingestion engine that pulls from **15 verified data sources** across a rolling 48-hour window, normalizes heterogeneous schemas, classifies categories using Google Gemini Flash, and serves the results on a fast, responsive map interface with zero client-side tracking.
...
[ Ingestion & Normalization Layer (NestJS 11) ]
  - Modular Provider Adapters with isolated error handling
...
[ Persistence Layer ]
  ...
  - Prisma ORM with atomic batch upserts
          ...
          ▼
[ REST API & Cache ]
  - GET /api/v1/events?date=today|tomorrow|all&free=true
...
- Pulls from REST APIs, GraphQL endpoints, Schema.org JSON-LD, and structured HTML parsers across **14 verified providers**.
...
### 5. Privacy-First Proximity Awareness & Hosting
...
### 6. Architectural Light & Dark Mode
...
- `provider` (string) — Filter by source provider (`STADT_WIEN`, `VIENNA_CLUBS`, `LUMA`, `RESIDENT_ADVISOR`, `OH_SCHON_HELL`, `CAPEET`, `EINTRITT_FREI`, `KULTURSOMMER`, `GOODNIGHT`, `EVENTS_AT`, `TICKETMASTER`, `EVENTBRITE`, `EVENTFROG`).
...
The project maintains comprehensive test suites with **127 automated tests** across both backend and frontend:

```bash
# Run backend test suite (NestJS / Jest) — 110 tests
npm run test

# Run frontend test suite (Next.js / Vitest) — 17 tests
cd frontend && npm run test

# Run full project test suite (127 tests)
